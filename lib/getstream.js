  ...
  return matches.reduce((acc, match) => {
    switch (typeof(match)) {
      ...
      case 'boolean':
        ...
        return [
          ...acc,
          ...generateFilterOnlyPaths(omit(query, term), path, store),
        ];
      case 'object':
        path = copyPath(path);
        if (match.lt) path.filter.push(`quad['${term}'] < '${match.lt}'`);
        if (match.lte) path.filter.push(`quad['${term}'] <= '${match.lte}'`);
        if (match.gt) path.filter.push(`quad['${term}'] > '${match.gt}${store.boundary}'`);
    ...
    switch (typeof(match)) {
      ...
      case 'boolean':
        if (canAddToIndexConditions(path)) {
          ...
          path.gt.push(match);
          path.gte = true;
          return [...acc, ...generatePaths(omit(query, term), path, terms.slice(1), store)];
        }
        return generateFilterOnlyPaths(query, path, store);
      case 'object':
        if (canAddToIndexConditions(path)) {
