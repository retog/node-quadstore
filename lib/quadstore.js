...
const MatchingFilterStream = require('./MatchingFilterStream');
...
class QuadStore extends events.EventEmitter {
  ...
  getStream(matchTerms, opts) {
    ...
    assert(_.isObject(matchTerms), 'The "matchTerms" argument is not a function..');
    assert(_.isObject(opts), 'The "opts" argument is not an object.');
    // const pattern = this._termsToPattern(matchTerms, {
    ...
    //   limit: opts.limit
    // };
    const [query, firstFilterStream, lastFilterStream] = this._termsToQuery(matchTerms);
    ...
    if (firstFilterStream && lastFilterStream) {
      valueStream.pipe(firstFilterStream);
      filteredStream = lastFilterStream;
    } else {
      filteredStream = valueStream;
    }

    let offsetStream;

    if (opts.offset) {
      offsetStream = filteredStream.pipe(this._createOffsetStream(opts.offset));
    } else {
      offsetStream = filteredStream;
    }
  ...
  _filtersToQuery(filters, opts) {
    ...
    if (_.isNil(query.lt) && _.isNil(query.lte)) query.lte = '';
    return query;
  }
  ...
  _termsToQuery(terms, opts) {
    ...
    let basePattern = '';
    let filterQuery;
    let firstFilterStream = new stream.PassThrough({ objectMode: true });
    let lastFilterStream = firstFilterStream;
    if (Array.isArray(terms.subject)) {
      filterQuery = this._filtersToQuery(terms.subject);
      if (terms.predicate) {
        if (terms.object) {
          if (terms[contextKey]) {
            // predicate, object, [contextKey]
            // POG(S)
            ...
            lastFilterStream = lastFilterStream.pipe(new MatchingFilterStream({ predicate: terms.predicate }))
            // predicate, object
            // OS + FILTERS(P)
          }
        } else {
          if (terms[contextKey]) {
            // predicate, [contextKey]
      ...
      if (terms.subject) {
        if (terms.predicate) {
          if (terms[contextKey]) {
            // subject, predicate, [contextKey]
            // OS + FILTERS
            basePattern = this._termsToPattern({subject: terms.subject, predicate: terms.predicate}, opts);
            lastFilterStream = lastFilterStream.pipe(new MatchingFilterStream({[contextKey]: terms[contextKey]}));
          } else {
            // subject, predicate
            // SPOG
            basePattern = this._termsToPattern({subject: terms.subject, predicate: terms.predicate}, opts);
          ...
          if (terms[contextKey]) {
            ...
            lastFilterStream = lastFilterStream.pipe(new MatchingFilterStream({subject: terms.subject, [contextKey]: terms[contextKey]}))
          } else {
            // subject
            basePattern = 'OS' + this.separator;
            lastFilterStream = lastFilterStream.pipe(new MatchingFilterStream({subject: terms.subject}));
          }
        }
      } else {
        if (terms.predicate) {
          if (terms[contextKey]) {
            ...
            // MISSING
            basePattern = 'OS' + this.separator;
            lastFilterStream = lastFilterStream.pipe(new MatchingFilterStream({predicate: terms.predicate, [contextKey]: terms[contextKey]}));
          } else {
            // predicate
            // POG
    ...
    return [query, firstFilterStream, lastFilterStream];
  }
