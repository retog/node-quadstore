        ...
        if (term.language) {
          ...
          switch(datatype) {
            ...
            case XSD + 'unsignedByte':
              const floatValue = parseFloat(term.value);
              if (floatValue >= MIN_INTEGER && floatValue <= MAX_INTEGER) {
                return `[number:${numberToString(parseFloat(term.value))}]${n3u.createLiteral(term.value, datatype)}`;
              }
            case XSD + 'date':
            case XSD + 'dateTime':
              const dateValue = new Date(term.value).valueOf();
              if (dateValue >= MIN_INTEGER && dateValue <= MAX_INTEGER) {
