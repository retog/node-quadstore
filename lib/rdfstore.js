...
class RdfStore extends QuadStore {
  ...
  getStream(matchTerms, opts) {
    ...
    if (matchTerms.subject) importedMatchTerms.subject = this._importTerm(matchTerms.subject, false, true);
    if (matchTerms.predicate) importedMatchTerms.predicate = this._importTerm(matchTerms.predicate, false, true);
    if (matchTerms.object) importedMatchTerms.object = this._importTerm(matchTerms.object, false, true);
    if (matchTerms.graph) importedMatchTerms.graph = this._importTerm(matchTerms.graph, true, true);
  ...
  _importTerm(term, isGraph, rangeBoundary = false) {
    return serialization.importTerm(term, isGraph, this._defaultContextValue, rangeBoundary);
  }
